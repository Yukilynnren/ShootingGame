...
namespace GameProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    if (bears[i].Active && bears[j].Active)
                    {
                        ...
                        if (info != null && info.FirstOutOfBounds)
                        {
                            bears[i].Active = false;
                        }
                        else if (info != null && !info.FirstOutOfBounds)
                        {
                            bears[i].Velocity = info.FirstVelocity;
                            bears[i].DrawRectangle = info.FirstDrawRectangle;
                            teddyBounce.Play();
                        }
                        if (info != null && info.SecondOutOfBounds)
                        {
                            bears[j].Active = false;
                        }
                        else if(info != null && !info.SecondOutOfBounds)
                        {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.DrawString(font, scoreString, GameConstants.SCORE_LOCATION, Color.Wheat);
